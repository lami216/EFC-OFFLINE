...
export function RegisterPage({data,user}:{data:Bootstrap;user:UserSession}){
  ...
  const branches=data.branches.filter(x=>x.active&&(!user.branchId||x.id===user.branchId));
  ...
  const specialties=data.specialties.filter(x=>x.active&&(!branchId||!data.specialtyBranches.some(m=>m.specialtyId===x.id)||mappedIds.has(x.id)));
  ...
  useEffect(()=>{if(branchId&&specialtyId){api.nextRegister(branchId,specialtyId).then(setNext).catch(()=>setNext(undefined))}else setNext(undefined)},[branchId,specialtyId]);
  useEffect(()=>{if(specialtyId&&!specialties.some(x=>x.id===specialtyId))setValue('specialtyId','')},[branchId]);
