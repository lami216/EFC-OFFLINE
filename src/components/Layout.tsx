...
const allLinks:[string,string,typeof UserPlus,Role[]][]=[
  ...
  ['/finance','المالية',Landmark,['ADMIN','FINANCE']],
  ['/ledger','اليومية',NotebookTabs,['ADMIN','FINANCE']],
...
export function Layout({data,user,onLogout}:{data:Bootstrap;user:UserSession;onLogout:()=>void}){
  ...
  const branch=user.role==='ADMIN'?'جميع الفروع':data.branches.find(x=>x.id===user.branchId)?.name||'جميع الفروع';
  useEffect(()=>{const key=(e:KeyboardEvent)=>{if(e.ctrlKey&&e.key.toLowerCase()==='n'&&(user.role==='ADMIN'||user.role==='REGISTRAR')){e.preventDefault();nav('/register')}if(e.ctrlKey&&e.key.toLowerCase()==='f'){e.preventDefault();nav('/students')}};window.addEventListener('keydown',key);return()=>window.removeEventListener('keydown',key)},[nav,user.role]);
